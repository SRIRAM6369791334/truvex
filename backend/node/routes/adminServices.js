...
function normalizeService(row = {}) {
  return {
    ...
    icon_name: row.icon_name || '', image: row.image || '',
    images: parseArray(row.images), features: parseArray(row.features),
    benefits: parseArray(row.benefits), process_steps: parseArray(row.process_steps),
    stats: parseArray(row.stats), specs: parseObject(row.specs),
    delivery_info: row.delivery_info || '', moq: Number(row.moq || 1),
...
function servicePayload(body) {
  ...
  return {
    title, slug: String(body.slug || slugFrom(title)).trim(),
...
function applyFiles(payload, files) {
  ...
  if (files?.gallery_files?.length) {
    const newImages = files.gallery_files.map((f) => `/uploads/${f.filename}`);
...
router.post('/', serviceUpload, async (req, res, next) => {
  try {
    ...
    const result = await queryResult(
      req.app.locals.db,
      `INSERT INTO services (title, slug, description, long_description, price, price_unit, in_stock, icon_name, image, images, features, benefits, process_steps, stats, specs, delivery_info, moq, category_id, subcategory_id, is_active, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [payload.title, payload.slug, payload.description, payload.long_description, payload.price, payload.price_unit, payload.in_stock, payload.icon_name, payload.image, payload.images, payload.features, payload.benefits, payload.process_steps, payload.stats, payload.specs, payload.delivery_info, payload.moq, payload.category_id, payload.subcategory_id, payload.is_active, payload.sort_order],
    );
    return res.status(201).json({ data: { id: result.insertId }, message: 'Service created.' });
  } catch (error) { return next(error); }
});
...
router.patch('/:id', serviceUpload, async (req, res, next) => {
  try {
    ...
    const result = await queryResult(
      req.app.locals.db,
      `UPDATE services SET title=?, slug=?, description=?, long_description=?, price=?, price_unit=?, in_stock=?, icon_name=?, image=?, images=?, features=?, benefits=?, process_steps=?, stats=?, specs=?, delivery_info=?, moq=?, category_id=?, subcategory_id=?, is_active=?, sort_order=? WHERE id=?`,
      [payload.title, payload.slug, payload.description, payload.long_description, payload.price, payload.price_unit, payload.in_stock, payload.icon_name, payload.image, payload.images, payload.features, payload.benefits, payload.process_steps, payload.stats, payload.specs, payload.delivery_info, payload.moq, payload.category_id, payload.subcategory_id, payload.is_active, payload.sort_order, req.params.id],
    );
