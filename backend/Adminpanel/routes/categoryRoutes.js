...
function categoryPayload(body, file) {
  ...
  return {
    name,
    slug: String(body.slug || slugFrom(name)).trim(),
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const grouped = subcategories.reduce((acc, subcategory) => {
      ...
      acc[subcategory.category_id].push(normalizeSubcategory(subcategory));
      return acc;
    }, {});
  ...
  try {
    ...
    return res.json({
      data: {
        ...
        subcategories: subcategories.map(normalizeSubcategory),
      },
    });
  } catch (error) {
    return next(error);
...
router.post('/', upload.single('image_file'), async (req, res, next) => {
  try {
    ...
    if (!payload.name) {
      return res.status(400).json({ error: 'Name is required.', fields: { name: 'Name is required.' } });
    }

    if (payload.trending && !(await assertTrendingLimit(req.app.locals.db))) {
      return res.status(409).json({
        error: 'You can only select up to 6 categories to show on the homepage. Please deselect an existing one first.',
      });
    }
    ...
    if (!payload.name || !payload.slug) {
      return res.status(400).json({
        ...
        fields: {
          ...
          slug: payload.slug ? undefined : 'Slug is required.',
        },
      });
    }
...
router.patch('/:categoryId/subcategories/:subcategoryId', upload.single('sub_image_file'), async (req, res, next) => {
  try {
    ...
    // Only update image if a new file was uploaded; otherwise keep existing
    const imageSql = req.file
      ? ', image = ?'
      : '';
    ...
    const result = await queryResult(
      ...
      `UPDATE subcategories
       SET name = ?, slug = ?, description = ?, is_active = ?, sort_order = ?${imageSql}
